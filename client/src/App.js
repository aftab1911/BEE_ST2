...
function App() {
  ...
  useEffect(() => {
    const getItemsList = async () => {
      try {
        const res = await axios.get(`${window.location.origin}/api/items`);
  ...
  const deleteItem = async (id) => {
    try {
      await axios.delete(`${window.location.origin}/api/item/${id}`);
  ...
  const updateItem = async (e) => {
    ...
    try {
      await axios.put(`${window.location.origin}/api/item/${isUpdating}`, updatedItem);
